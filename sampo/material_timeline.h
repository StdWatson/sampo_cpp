...
private:
	//map<string, int> _capasity;
	map<std::string, map<std::string, int>> _resource_sources;
	map<string, vector<pair<Time, int>>> _timeline;
	map<string, int> _capacity;
...
public:
	...
	pair< MaterialDelivery*, Time> supply_resources(string work_id, Time deadline,
													vector<Material*> materials, bool simulate, Time min_supply_start_time = Time(0));
	bool can_schedule_at_the_moment(string id, Time start_time, vector< Material*> materials, int batch_size);
	Time find_min_material_time(string id, Time start_time, vector< Material*> materials, int batch_size);
	tuple<vector< MaterialDelivery*>, Time, Time> deliver_materials(string id, Time start_time, Time finish_time,
																	vector< Material*> materials, int batch_size);
	map<std::string, map<std::string, int>> resource_sources();
};